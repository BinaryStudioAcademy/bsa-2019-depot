...
const getUsersOrganizations = async (userId) => {
  const orgsIds = await OrgUserRepository.getUsersOrganizations(userId);
  const orgNames = orgsIds.map(org => getUserById(org.orgId));
  return Promise.all(orgNames);
};
